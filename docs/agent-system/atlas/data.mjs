...
export const NODES = [
  ...
  },
  {
    ...
    cond: [{ q: 'Should remaining stationary receive a generic penalty?', r: 'Not yet: A6’s unchanged-position turns were directional inputs and may include attacks or digging. Classify outcomes first (2026-08-24).' }, { q: 'How should player damage be measured?', to: 'Use authoritative before/after health loss bounded by pre-turn health; retain raw attack magnitude only as diagnostic metadata. Historical A8 batches prove raw lethal overkill can create impossible reward and critic shocks.' }, { q: 'Does item_collected measure equipment?', to: 'No. The current bridge emits it only from objectCurrency, so historical item-pickup metrics are coin-pile transactions. Split currency and equipment events before using item competence as a promotion gate.' }, { q: 'What is Reward V5?', to: 'Define it only after truncation semantics, actual-health damage, event semantics, and critic observability are corrected.' }],
  },
  {
  ...
  },
  {
    ...
    how: `<strong>Version lineage.</strong><br><code>Initial recurrent PPO</code> used the same core defaults but synchronous collection.<br><code>Architecture A2</code> established exact 32-step LSTM replay and architecture-checked checkpoints.<br><code>Async runtime</code> preserved the algorithm while feeding versioned actor fragments.<br><code>Current diagnostics</code> record value loss and one pre-clipping gradient snapshot per representation group, but omit explained variance and value/return/advantage distributions. Two A8 updates reached value losses 183–190 alongside impossible raw-overkill damage targets, so critic health is not presently qualified. Rollout cadence remains 128×N transitions per update. Defaults: γ .99, GAE .95, clip .2, lr 3e-4, entropy .01, value .5, grad norm .5, four epochs.`,
    ...
    cond: [{ q: 'Is one update per 1,024 steps optimal at eight workers?', to: 'Benchmark rollout length, epochs, minibatch chunks, and GPU utilization while holding total environment steps and evaluation seeds fixed.' }, { q: 'Is γ=.99 aligned with multi-floor play?', to: 'No evidence establishes that. Its effective horizon is about 100 turns and γ^1000 is roughly 4.3e-5, while failed episodes last 3,000–5,000 turns. Test horizon only after logging explained variance and target scale, with matching potential-shaping discount.' }, { q: 'Is the current critic healthy?', to: 'Unknown: value loss alone is scale-dependent. Add explained variance, predicted-value, return-target, raw-advantage, and gradient-norm distributions before changing gamma or adding PopArt.' }, { q: 'May PPO train on a partial healthy fleet?', r: 'No; fixed capacity and same-version rollout integrity take precedence over silently continuing with fewer workers (2026-08-24).' }],
  },
  {
