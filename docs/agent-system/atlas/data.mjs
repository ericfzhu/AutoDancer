export const META = {
  ...
  stats: [
    { k: 'Current default', v: 'schema 10 · Architecture A2' },
  ...
  onePara: `AutoDancer is a recurrent PPO agent that learns Bard by acting in real Crypt of the NecroDancer processes. Python owns a fixed fleet of isolated game workers, exchanges actions and complete transitions with a Lua/native named-pipe bridge, constructs player-visible observations and explicit floor memory, and batches experience for a shared actor-critic. The outer experiment loop—not shaped return—decides whether a reward or architecture version is better using gameplay on unseen seeds. A2 with Reward V2 remains the measured baseline. EXP-0008 showed that extending both A2 and materially active A8 to 250,880 transitions did not produce held-out progress, so adaptation horizon is rejected. EXP-0009 showed that stochastic execution reproducibly reaches Floor 2 and once reached Floor 3, while deterministic execution collapses, but it did not reach Zone 2. Time-limit bootstrapping, actual-health damage scoring, critic diagnostics, and descent-source attribution are now corrected for the next causal training comparison.`,
  costModel: [
...
export const NODES = [
  ...
  },
  {
    ...
    cond: [{ q: 'How much does the trained policy rely on its LSTM?', to: 'Run hidden-state ablations and delayed-information probes on fixed seeds without changing rewards.' }, { q: 'Does a persistent hidden state provide unlimited learnable memory?', to: 'No. State persists during play, but 32-step truncated BPTT biases gradients toward local dependencies. Compare 32/64/128-step chunks on fixed recurrent batches before changing the recurrent architecture.' }, { q: 'Should the map be left to the LSTM instead?', r: 'No; explicit player-like spatial memory removes an avoidable information burden, while the LSTM handles temporal context (2026-08-24).' }],
  },
  {
    ...
    what: 'A stateful tracker deduplicates and caps exploration, combat, item, and navigation credit. Task reward, shaping reward, and gameplay outcomes are recorded separately; held-out gameplay selects policies. Player damage is normalized to the authoritative before/after health loss bounded by pre-turn health; raw attack magnitude remains diagnostic metadata.',
    ...
    cond: [{ q: 'Should remaining stationary receive a generic penalty?', r: 'Not yet: unchanged-position turns can include attacks, digging, or blocked movement; outcome-specific evidence is required (2026-08-24).' }, { q: 'How is player damage measured?', r: 'The live adapter now scores before/after health loss bounded by pre-turn health and retains raw attack magnitude plus event count only as diagnostics (2026-08-27).' }, { q: 'How are items and currency distinguished?', r: 'Lua emits currency_collected from the currency hook; Python derives item_collected only from positive inventory deltas. Reports separate item units, unique acquired types, currency transactions, and currency value (2026-08-27).' }, { q: 'What is Reward V5?', to: 'Define it only after corrected semantics produce critic-health and progression evidence; do not change reward and learning integrity in the same comparison.' }],
  },
  {
  ...
  },
  {
    ...
    how: `<strong>Version lineage.</strong><br><code>Initial recurrent PPO</code> used the same core defaults but synchronous collection.<br><code>Architecture A2</code> established exact 32-step LSTM replay and architecture-checked checkpoints.<br><code>Async runtime</code> preserved the algorithm while feeding versioned actor fragments.<br><code>Learning-integrity correction</code> distinguishes true terminations from time limits, bootstraps each truncated final observation while stopping GAE across reset, and records explained variance, value/return/raw-advantage/reward scale, and total pre-clipping gradient norm. Two historical A8 updates reached value losses 183–190 alongside impossible raw-overkill damage targets; new runs can now determine whether the corrected critic is healthy. Rollout cadence remains 128×N transitions per update. Defaults: γ .99, GAE .95, clip .2, lr 3e-4, entropy .01, value .5, grad norm .5, four epochs.`,
    ...
    cond: [{ q: 'Is one update per 1,024 steps optimal at eight workers?', to: 'Benchmark rollout length, epochs, minibatch chunks, and GPU utilization while holding total environment steps and evaluation seeds fixed.' }, { q: 'Is γ=.99 aligned with multi-floor play?', to: 'No evidence establishes that. Its effective horizon is about 100 turns and γ^1000 is roughly 4.3e-5, while failed episodes last 3,000–5,000 turns. Test horizon only after reviewing the newly logged explained variance and target scale, with matching potential-shaping discount.' }, { q: 'Is the corrected critic healthy?', to: 'The required diagnostics now exist, but health remains an empirical question until a corrected training run reports stable explained variance, bounded targets, and gradients.' }, { q: 'May PPO train on a partial healthy fleet?', r: 'No; fixed capacity and same-version rollout integrity take precedence over silently continuing with fewer workers (2026-08-24).' }],
  },
  {
...
export const CH = [
  ...
  },
  {
    ...
    lede: 'Stochastic execution reveals useful movement but not Zone 2; corrected learning semantics are the next isolated variable.',
    story: `<p>EXP-0008 rejected the claim that A8 only needed a longer adaptation horizon. EXP-0009’s completed A2-frozen trials showed a real execution gap: deterministic play remained on Floor 1, both sampled streams repeated Floor 2 on seeds 57004/57005/57009, and one stream reached Floor 3 on seed 57019—but none reached Zone 2. The active continuation trial was mistakenly interrupted after its owned workers were queried under the wrong executable name; there is no evidence of a launcher failure, and the partial reports are not overstated. <mark>The learner now treats time limits as truncations with final-state bootstrap, scores actual health loss instead of lethal overkill, exposes critic target/quality diagnostics, distinguishes staircase/trapdoor/unknown descents, and separates inventory acquisitions from currency.</mark> The next experiment holds architecture and reward fixed to isolate whether these corrections improve learning before curriculum or reward V5 is introduced.</p>`,
