...
export const NODES = [
  ...
  },
  {
    ...
    cond: [{ q: 'Should remaining stationary receive a generic penalty?', r: 'Not yet: unchanged-position turns can include attacks, digging, or blocked movement; outcome-specific evidence is required (2026-08-24).' }, { q: 'How is player damage measured?', r: 'The live adapter now scores before/after health loss bounded by pre-turn health and retains raw attack magnitude plus event count only as diagnostics (2026-08-27).' }, { q: 'Does item_collected measure equipment?', to: 'No. The current bridge emits it only from objectCurrency, so historical item-pickup metrics are coin-pile transactions. Split currency and equipment events before using item competence as a promotion gate.' }, { q: 'What is Reward V5?', to: 'Define it only after corrected semantics produce critic-health and progression evidence; do not change reward and learning integrity in the same comparison.' }],
  },
  {
