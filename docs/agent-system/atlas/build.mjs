...
const cnt = { open: 0, routed: 0, res: 0 };
NODES.forEach((n) =>
  (n.cond || []).map(Q).forEach((c) =>
    c.r ? cnt.res++ : c.to ? cnt.routed++ : cnt.open++
  )
);
...
function buildSystemMd() {
  ...
  out.push(META.intro, '');
  out.push(
    `_Question status: **${cnt.open} open · ${cnt.routed} routed · ${cnt.res} resolved**._`,
    '',
  );
  ...
  index.forEach(([id, code, c]) =>
    out.push(
      c.r
        ? `- ~~**${id}**~~ (${code}) ✓ ${md(c.r)}`
        : c.to
          ? `- **${id}** (${code}) ${md(c.q)} → _${md(c.to)}_`
          : `- **${id}** (${code}) ${md(c.q)}`,
    ),
  );
  out.push('');
...
console.log(`built SYSTEM.md + atlas.html · ${cnt.open} open · ${cnt.routed} routed · ${cnt.res} resolved · ${NODES.length} structures · ${DECISIONS.length} decisions`);
