"""Execute the immutable EXP-0041 pilot with hard budgets and no policy selection."""
...
OUT = ROOT / "runs/stable-navigation-learning-exp0041"
...
def main():
    ...
    spec = store.load("EXP-0041")
    reference = ROOT / spec.data["source"]["frozen_reference"]
    from analyze_bounded_navigation_comparison import analyze

    assert analyze(reference)["validated"]
    ...
    files += [
        ...
        reference / "validated-analysis.json",
        spec.path,
        ROOT / "tools/audit_first_update.py",
    ]
    files += list((reference / "evaluations").glob("*/report.json"))
    ...
    reports = {}
    try:
        store.set_status("EXP-0041", "running")
        for stage in ("frozen", "pilot"):
            ...
            for stream in streams:
                ...
                if stage == "frozen":
                    previous = reference / "evaluations" / f"navigation-{stream}" / "report.json"
                    shutil.copy2(previous, path)
                    invocation = {
                        "stage": name,
                        "exit_code": 0,
                        "elapsed_seconds": 0,
                        "reused_report": str(previous),
                        "reused_report_sha256": sha256_file(previous),
                    }
                    atomic_json(OUT / f"{name}-invocation.json", invocation)
                else:
                    invocation = execute(
...
def train_child():
    ...
    from autodancer.training.ppo import RecurrentPPO
    from tools.audit_first_update import replay, restore_rng, rng_state

    original_update = RecurrentPPO.update

    def measured_update(algorithm, rollout):
        output = Path(sys.argv[sys.argv.index("--run-dir") + 1]).parent
        if algorithm.updates == 0:
            algorithm.save(output / "initial-learner.pt")
        if algorithm.updates in (0, 15):
            payload = {}
            ...
            torch.save(payload, output / f"rollout-update-{algorithm.updates + 1:02d}.pt")
        state = rng_state()
        before = replay(algorithm, rollout)
        restore_rng(state)
        assert before["logprob_max_abs_difference"] < 0.001, before
        metrics = original_update(algorithm, rollout)
        state = rng_state()
        after = replay(algorithm, rollout)
        restore_rng(state)
        metrics["pre_update_logprob_max_abs_difference"] = before["logprob_max_abs_difference"]
        metrics["post_update_full_batch_approx_kl"] = after["sampled_action_approx_kl"]
        metrics["post_update_full_batch_clip_fraction"] = after["ratio_outside_clip_fraction"]
        metrics["post_update_logprob_max_abs_difference"] = after["logprob_max_abs_difference"]
        assert all(math.isfinite(float(value)) for value in after.values())
        return metrics

    RecurrentPPO.update = measured_update
    sys.argv = ["autodancer.training.train", *sys.argv[2:]]
